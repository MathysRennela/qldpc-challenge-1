# [[288,8,36]] — bivariate-bicycle weight-27 candidate
...
The final submission CLI used 8,000 RIS trials and obtained d_X<=42 and d_Z<=41. The independent CI gate later found a reproducible weight-36 X-type logical, so the corrected claim is d_X<=36 and d_Z<=41, with d<=36. This is not an exact-distance claim.
...
Rebuild the exact checks from `codes/288-8-36.json`; its X/Z support lists and X witness are the complete reproducible artifact. The construction is a bivariate bicycle on Z_12 x Z_12 with the listed supports. The corrected X witness has weight 36; the final original witness search used 8,000 RIS trials.
