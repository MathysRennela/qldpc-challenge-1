...
Automated check-deletion / degree-one-removal campaign in the repository's Python kit. `provenance.model` is deliberately left unset: the campaign did not record which model drove it, and guessing one would be false provenance. Verification used `verify/validate_candidate.py`, the submission CLI in `cli/qldpc.py` with its default 20000-trial witness search and 2000000-trial accelerator, and `verify/check_prose.py`. Local CPU only, 10 cores, no paid compute.
