# [[360,8,55]] — bivariate-bicycle weight-29 candidate
...
The initial screen reported d<=56. The submission CLI reran both CSS directions with 8,000 RIS trials and obtained d_X<=55 and d_Z<=57, correcting the claim to d<=55. The full local verifier passed the CSS, rank, weight, witness, and fresh refutation checks. This is not an exact-distance claim.
...
Rebuild the exact checks from `codes/360-8-55.json`; its X/Z support lists are the complete reproducible artifact. The construction is a bivariate bicycle on Z_20 x Z_9 with the listed supports. The final witness search used 8,000 RIS trials.
