#!/usr/bin/env python3
"""Probe dense-packed surface qubit additions (route 5, k=6 extension).

The [[101,5,5]] dense-packed code (arXiv:2511.06758) is reconstructed by
research/build_dense_surface.py.  This probe tries adding a data qubit at an
empty odd,odd site together with the checks at its 4 diagonal-neighbor
ancilla sites (extending existing checks where possible, adding new ones
otherwise), keeping CSS-valid variants.  It reports (n,k,d) with the kit's
exact GF(2) core (distance is an UPPER BOUND via surrogate).
...
def probe(trials=1500, seed=0):
    ...
    # data qubits are at (0.5,0.5),(1.5,0.5),... i.e. normalized (odd/2, odd/2)
    # empty candidate sites: odd,odd in the dense lattice within the bounding box
    ...
    for x in range(1, xmax + 1, 2):
        for y in range(1, ymax + 1, 2):
            if (x / 2, y / 2) in coord2idx:
                continue
            # diagonal-neighbor ancilla sites (x±1,y±1) must be occupied
            anc = [(x + dx, y + dy) for dx, dy in [(-1, -1), (-1, 1), (1, -1), (1, 1)]]
            # ancilla sites are at even,even or even,odd... in the dense lattice
            # ancillas are non-data sites; check they're in the site set
            # (allcoords maps site index -> (x,y) for occupied sites)
            site_set = set(allcoords)
            if not all(a in site_set for a in anc):
                continue
            # count how many of the new qubit's 4 diagonal data neighbors exist
