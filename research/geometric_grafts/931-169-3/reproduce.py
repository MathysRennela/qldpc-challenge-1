...
def payload(state):
    ...
    return {'n': int(hx.shape[1]), 'checks': {
        side: [np.flatnonzero(row).astype(int).tolist() for row in h]
        for side, h in [('X', hx), ('Z', hz)]},
        'coordinates': np.asarray(coords, dtype=float).tolist(), 'layers': 1}
...
def apply_move(state, move, cleanup):
    ...
    matches = [i for i, row in enumerate(h)
               if [original[int(j)] for j in np.flatnonzero(row)] == support]
...
def check_saved_witnesses(reconstruction, evidence):
    """Delegation to the repository validator; no new search is performed."""
    ...
    for case in evidence['cases']:
        ...
        proposals = [(side, case['initial']['distance'][side]['value'],
                      case['initial']['distance'][side]['witness']) for side in ('X', 'Z')]
...
def main():
    ...
    parser.add_argument('--all-cases', action='store_true', help='Rebuild all archived proposals and compare exact array fingerprints.')
    parser.add_argument('--check-witnesses', action='store_true', help='Validate saved supports with the unchanged repository witness checker; no search.')
    parser.add_argument('--compare', type=Path, help='Compare final check arrays and coordinates against a candidate document.')
    ...
    print(json.dumps({'n': final[0].shape[1], 'final_arrays_sha256': fingerprint(final),
        'additional_recipe_nodes_checked': checked, 'saved_witnesses_validated': witnesses,
        'distance_searches_performed': 0}, indent=2))
