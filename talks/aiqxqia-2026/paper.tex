...
%% TODO before submission: confirm the full author list and affiliations with
%% the group (the email thread includes Ben and Brad, whose surnames and
%% affiliations need to be confirmed), and add ORCIDs.
\author[1]{Vincent Russo}[email=vincentrusso1@gmail.com]
\author[1]{Mathys Rennela}
\author[1]{Farrokh Labib}
\author[1]{Seun Omonije}
\author[1]{William J. Zeng}
...
The system is of interest to this workshop in both directions of the AI and
quantum intersection: it is infrastructure that makes AI-driven quantum code
discovery auditable, and its accumulated history of refuted and confirmed
claims is itself a dataset about the reliability of heuristic verification,
a question AI methods for quantum error correction cannot avoid.
...
\paragraph{A code is not a single number.} A quantum code trades physical
...
\section{A leaderboard of cells, not one number}
...
against published codes rather than an empty table.

\section{Figures of merit, honestly labeled}
\label{sec:metrics}
...
post-processing~\cite{roffe2020decoding}, two genuinely different mechanisms.
...
refutation; a solver timeout leaves the claim honestly an upper bound.
...
improves, keeping the board an honestly finite-length benchmark rather than
...
an upper-bound claim deserves confidence? The accumulated record is
...
\begin{itemize}
    ...
    and the honest entry became $d\le 85$. The refutation arrived in
...
that genuinely independent search mechanisms (information-set sampling versus
