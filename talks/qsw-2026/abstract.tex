...
\begin{enumerate}[label=\arabic*.,leftmargin=*,nosep]
  ...
  checkable counterexample). Frontier claims also face an accelerated search at
  ${\sim}150\times$ the baseline budget whose finds count only after the
  trusted verifier validates the witness. The per-run seed is
...
group-algebra), coset, tile, and topological families, including a first odd-$k$ entry ($[[240,13,15]]$,
non-abelian two-block on $S_5$) and a $[[216,15,11]]$ planar bilayer code
leading the 2D-local weight-8 cell. It is seeded against a selection of published codes (the
