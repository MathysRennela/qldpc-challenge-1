...
as code discovery itself becomes automated: LLM- and agent-driven
searches~\cite{cruzbenito2026evolutionary} can propose candidate codes far
faster than humans can referee them, and an overstated distance is precisely
their characteristic failure mode. An adversarial, machine-checked gate closes
that loop, so the board doubles as the verification harness of an automated
research cycle: an agent proposes, the pipeline refutes or verifies, and only
survivors enter the record.
