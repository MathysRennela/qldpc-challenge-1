...
Each case isolates one gate so a failure points at the right place:
  ...
  1. a genuinely-new valid code PASSES (verifies, converges, not refuted, not a dup);
  2. an OVER-CLAIM (same checks, witnesses inflated) is caught by converge AND refute,
     and is *not* a duplicate -- so the failure is unambiguously the over-claim gate;
...
(validate_candidate) imports only verify/, never research/. An explicit seed + a
small converge ladder keep the run deterministic and fast.
...
def main():
    ...
    vg = V.validate_candidate(good, seed=SEED, converge_ladder=LADDER)
    check("passes", vg["passed"])
    check("verifies", vg["gates"]["verify"]["ok"])
    check("not refuted", not vg["gates"]["refute"]["refuted"])
    check("not over-claimed", not vg["gates"]["converge"]["over_claimed"])
    ...
    vo = V.validate_candidate(over, seed=SEED, converge_ladder=LADDER)
    check("does not pass", not vo["passed"])
    check("caught by converge (over_claimed)", vo["gates"]["converge"]["over_claimed"])
    check("caught by refute", vo["gates"]["refute"]["refuted"])
    check("NOT flagged a duplicate (isolates the over-claim gate)",
    ...
    vd = V.validate_candidate(dup, seed=SEED, converge_ladder=LADDER)
    ...
    vb = V.validate_candidate(broken, seed=SEED, converge_ladder=LADDER)
