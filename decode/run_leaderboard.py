...
P_REF = 0.04          # ranking noise rate
P_LOW = 0.02          # second point to show LER scaling
SHOTS = 6000
SEED = 11
...
for f in sorted(glob.glob(os.path.join(ROOT, "codes", "*.json"))):
    ...
    hi = logical_error_rate(HX, HZ, P_REF, shots=SHOTS, seed=SEED)
    lo = logical_error_rate(HX, HZ, P_LOW, shots=SHOTS, seed=SEED)
    results[slug] = {
        "n": n, "k": k,
        "per_logical_ler": round(hi["ler_per_logical"], 6),   # at P_REF (rank)
        "block_ler": round(hi["ler"], 6),
        "per_logical_ler_low": round(lo["ler_per_logical"], 6),  # at P_LOW
        "stderr": round(hi["stderr"], 6)}
    print(f"{slug}: k={k} per_logical@{P_REF}={hi['ler_per_logical']:.5f} "
          f"@{P_LOW}={lo['ler_per_logical']:.5f}", flush=True)

doc = {
    "protocol": {
        "noise": "code-capacity, independent X and Z",
        "p": P_REF, "p_low": P_LOW,
        "decoder": "BP+OSD (ldpc, osd_cs, order 10, max_iter 30)",
